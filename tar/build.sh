...
then
  echo "Patching .. "
  cd "$BUILDDIR"
  patch -p0 < ../patches/src.list.c.diff
  # No patches yet
  TASK=build
fi
...
then
  ...
  # configure
  ./configure --host=arm-unknown-nto-qnx6.5.0eabi --build=x86_64-apple-darwin --target=arm-unknown-nto-qnx6.5.0eabi --prefix="$DESTDIR" --disable-nls CC=arm-unknown-nto-qnx6.5.0eabi-gcc LIBS="-lsocket"
  make
  TASK=install
fi
