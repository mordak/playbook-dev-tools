...
then
  ...
  # configure 
  ./configure --host=arm-unknown-nto-qnx6.5.0eabi --build=x86_64-apple-darwin --target=arm-unknown-nto-qnx6.5.0eabi --prefix="$DESTDIR" --enable-threads=posix CC=arm-unknown-nto-qnx6.5.0eabi-gcc
  make
  TASK=install
fi
