...
then
  ...
  # configure 
  ./configure --host=arm-unknown-nto-qnx6.5.0eabi --build=x86_64-apple-darwin --target=arm-unknown-nto-qnx6.5.0eabi --prefix="$DESTDIR" --enable-threads=posix --disable-nls CC=arm-unknown-nto-qnx6.5.0eabi-gcc --enable-no-install-program=mknod,stat,uname
  make
  TASK=install
fi
